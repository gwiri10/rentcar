...
const getDateDiff = async () => {
    ...
    //기간별 요금 세팅
    //1. 비수기
    let rage1 = data.upData1 != undefined ? data.upData1 : '24/01/06 - 25/01/24'
    ...
    //5. 속해있는 곳이 없으면 비수기 요금으로 하겠음
    let flag = isDateInRangeResult || isDateInRangeResult2 || isDateInRangeResult3 || isDateInRangeResult4
    if (!flag) {
        if (diffDay == 1) {
            addPrice = data.addprice24_1 != undefined ? data.addprice24_1 : 5500
        } else if (diffDay == 2) {
    ...
    //환율 정보 가져오기 
    let querySnapshot = await fetchData("rate");
    let exchangeRate = 0;
    ...
    let beforePrice = Math.ceil(totalPrice * calRate * exchangeRate / 10) * 10;
    let afterPrice = totalPrice * (1-calRate)
